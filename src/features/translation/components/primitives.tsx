// Shared §7.7 UI primitives used across C1–C5.
// All five panels import from here; per-panel reinvention is prohibited.
...
/** Container used by every result panel (C1–C5). Keeps chrome consistent. */
...
// Left-stripe colour distinguishes source category (visual cue, paired with label text).
...
/** Small pill with a 4 px coloured left stripe and the humanised source label. */
...
type NotesProps = { text: string | null }

/** Renders notes text with inline citation tags styled in mono/subtle colour. */
export function NotesWithCitations({ text }: NotesProps) {
  if (!text) return null
