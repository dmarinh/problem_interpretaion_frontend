...
## 2026-05-01 — StandardizationSchema and range_clamps aligned with backend audit shape

**Context:** Five Zod errors surfaced on a live bread-query capture (ptm 6d8aa38); frontend rejected valid backend responses entirely.

**Lesson:** Four root causes across two schemas: (1) `StandardizationSchema.direction` typed non-nullable — `range_clamp` and `default_imputed` rules produce `null` because no bound is selected; (2) `before_value` missing `null` — `default_imputed` has no prior value; (3) `after_value` typed as `number` only — categorical fields like `organism` produce a string `"Salmonella"`; (4) `AuditCategoriesSchema.range_clamps` typed as `string[]` — backend has emitted structured `{ field_name, original_value, clamped_value, valid_min, valid_max, reason }` objects for some time. The frontend never caught up on (4) because `range_clamps` was always empty in earlier fixtures. The structured shape had been documented in the task brief but not reflected in the schema or rendering layer. Each fix had downstream TypeScript errors: `formatStdLabel` signature (`string | undefined` → `string | null | undefined`), `VerboseCategory` for range_clamps replaced with `RangeClampCategory`, and `pipelineStrip.test.ts` CLEAN_CATS / fixture (b) updated from string arrays to objects.

**Action:** When a backend audit category changes from string to structured object (or vice versa), check all four layers: (1) Zod schema, (2) TypeScript type export in `types.ts`, (3) rendering component (the `VerboseCategory` pattern assumes `string[]`; structured lists need their own component like `DefaultsImputedCategory`/`RangeClampCategory`), (4) test fixtures — both inline and JSON. Also check `pipelineStrip.test.ts` CLEAN_CATS, which mirrors the `AuditCategoriesSchema` shape and will silently become wrong when that schema changes. Use `z.tuple([z.number(), z.number()])` for min-max pairs (not `z.array(z.number())`); the tighter type catches backend regressions and is accurate to the domain.

**Reference:** `src/features/translation/api/schema.ts` (StandardizationSchema, RangeClampSchema, AuditCategoriesSchema), `src/features/translation/components/AuditChecks.tsx` (RangeClampCategory), `src/features/translation/utils/format.ts` (formatStdLabel), `src/features/translation/utils/__tests__/pipelineStrip.test.ts` (CLEAN_CATS, fixture b).
